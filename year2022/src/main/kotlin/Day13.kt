object Day13 : Day<Int> {
    data class Packet(val value: String) : Comparable<Packet> {
        override fun compareTo(other: Packet): Int {
            ...
            val r = right.first()
            return when {
                l == r -> compareValuesBy(left.drop(1), right.drop(1), ::Packet)
                l == ']' -> -1
                r == ']' -> 1
                l == '[' -> compareValuesBy(left.drop(1), "$r]" + right.drop(1), ::Packet)
                r == '[' -> compareValuesBy("$l]" + left.drop(1), right.drop(1), ::Packet)
                else -> l compareTo r
            }
        }
    }

    private fun String.toPackets() = lines().filter { it.isNotEmpty() }.map { it.replace("10", "A").let(::Packet) }
